...
def load_rplanhg_data(
    ...
    if deterministic:
        loader = DataLoader(
            dataset, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False, collate_fn=collate_fn
        )
    else:
        loader = DataLoader(
            dataset, batch_size=batch_size, shuffle=True, num_workers=0, drop_last=False, collate_fn=collate_fn
...
class RPlanhgDataset(Dataset):
    def __init__(self, set_name, analog_bit, target_set, non_manhattan=False):
        ...
        if os.path.exists(f'processed_rplan/rplan_{set_name}_{target_set}.npz'):
            ...
            np.savez_compressed(f'processed_rplan/rplan_{set_name}_{target_set}', graphs=np.array(self.graphs, dtype=object), houses=np.array(self.houses, dtype=object),
                    door_masks=np.array(self.door_masks, dtype=object), self_masks=np.array(self.self_masks, dtype=object), gen_masks=np.array(self.gen_masks, dtype=object))
            if self.set_name=='train':
